...
> Последнее обновление: 2026-06-12 (итерация 2: защита исследования, аудит механики)
...
**Состояние дороги (заменило _cv_offset):**
```gdscript
_road_active: bool   // герой стоит на дороге, не в локации
_road_path:   Path2D // активная Path2D
_road_dest:   String // локация на дальнем конце
_road_fwd:    bool   // направление вдоль кривой
_road_offset: float  // смещение героя вдоль кривой (native)
```

**Ключевые методы Main.gd:**
```gdscript
_route_path_for(a, b)                  // Path2D для пары локаций (dict)
_try_road_click(world_pos)             // выбор дороги + запуск движения
_start_road_move(path, dest, off, fwd, wp) // частичное движение к клику
